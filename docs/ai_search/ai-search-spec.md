...
- When `kAiSearchEnabled` is **true**:
  1. Naming (R12). Bottom nav label stays "Cari" (same as flag off). Home "Akses Cepat" tile stays "Cari". Screen header title is "Temukan di Al-Qur'an" with subtitle "Dari ayat, tafsir, doa, dan Asmaul Husna" (same header widget and subtitle style as the other tabs). Search-field placeholder "Cari sabar, rezeki, atau 2:255".
  2. Empty-query landing: section label "Coba" above four example **tags** (`sabar`, `rezeki`, `doa untuk orang tua`, `hati gelisah` — UI examples only) that wrap so every tag is fully visible, fill the field, and run the query. Tags are compact pills (not 2x2 cards, not default ActionChip). Below them, section "Cara ketik" with three tappable type-in rows (`2:255`, `juz 30`, `halaman 5`) that fill those samples — teaching how to type a reference, not browsing Baca's Surah/Juz/Halaman lists. No Tanya landing card, no "Atau cari lebih spesifik" heading, no six browse cards, and no leftover decorative empty mark.
...
| Thresholds after G1 | | |
| Enable `kAiSearchEnabled` | 2026-09-24 | enabled for v47; Phase G gate waived by maintainer, Phase D deferred |
